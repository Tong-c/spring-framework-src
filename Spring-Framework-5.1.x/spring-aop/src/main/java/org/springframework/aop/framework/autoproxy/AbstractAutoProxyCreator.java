...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
	...
	protected Object wrapIfNecessary(Object bean, String beanName, Object cacheKey) {
		...
		// 如果包含通知，创建当前bean对应的代理对象

		// 4.获取当前bean对应的Advices和Advisors（增强方法）
		Object[] specificInterceptors = getAdvicesAndAdvisorsForBean(bean.getClass(), beanName, null);

		// 	如果specificInterceptors不为null的话（存在增强方法），则需要针对增强创建代理
		if (specificInterceptors != DO_NOT_PROXY) {
			// 标记当前bean已经处理过，即保存当前bean在advisedBeans缓存中，并且值为true
			this.advisedBeans.put(cacheKey, Boolean.TRUE);

			// 5.创建代理对象
			// new SingletonTargetSource(bean): 目标对象，方便获取被代理的原始对象
			Object proxy = createProxy(bean.getClass(), beanName, specificInterceptors, new SingletonTargetSource(bean));

			// 6.创建完代理后，将cacheKey -> 代理类的class放到proxyTypes缓存中
			// private final Map<Object, Class<?>> proxyTypes = new ConcurrentHashMap<>(16);
			this.proxyTypes.put(cacheKey, proxy.getClass());
			// 给容器中返回当前bean增强之后的代理对象, 以后容器中获取到的就是这个bean的代理对象，执行目标方法的时候，代理对象就会执行通知方法的流程
			return proxy;
		}
	...
	protected Object createProxy(Class<?> beanClass, @Nullable String beanName, @Nullable Object[] specificInterceptors, TargetSource targetSource) {
		...
		// 检查proxyTargetClass属性，判断对于给定的bean使用Cglib动态代理还是JDK动态代理（如@EnableAspectJAutoProxy(proxyTargetClass = true)：true的话代表强制使用Cglib动态代理）
		if (!proxyFactory.isProxyTargetClass()) {
			// 检查preserveTargetClass属性的值是否为true，判断beanClass是Cglib动态代理还是JDK动态代理
		...
		// 将当前bean对应的拦截器链转换成Advisor增强器
		Advisor[] advisors = buildAdvisors(beanName, specificInterceptors);
		// 将advisors添加到代理工厂proxyFactory中
		proxyFactory.addAdvisors(advisors);
		// 设置被代理类对象（即目标对象）
		proxyFactory.setTargetSource(targetSource);
		// 自定义ProxyFactory，这是Spring提供的一个扩展点，留给子类实现
		customizeProxyFactory(proxyFactory);
		// 控制代理工厂被配置之后，是否还允许修改通知，默认为false，即不允许修改代理的配置
