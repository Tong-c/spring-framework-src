...
class ConstructorResolver {
	...
	public BeanWrapper autowireConstructor(String beanName, RootBeanDefinition mbd,
		...
		// 2、解析用于实例化的构造函数参数
		// explicitArgs：通过 getBean 方法显式传入的参数值，或者使用 bean 定义中的构造函数参数值
		if (explicitArgs != null) {
			// 如果explicitArgs不为空，则构造函数的参数直接使用explicitArgs
			argsToUse = explicitArgs;
		} else {
			// 如果explicitArgs为空，将尝试从缓存中获取已经解析过的构造函数参数
			Object[] argsToResolve = null;
			synchronized (mbd.constructorArgumentLock) {
				// 从resolvedConstructorOrFactoryMethod缓存中获取已经解析好的构造方法或者工厂方法
				constructorToUse = (Constructor<?>) mbd.resolvedConstructorOrFactoryMethod;
				if (constructorToUse != null && mbd.constructorArgumentsResolved) {
					// Found a cached constructor...
					// 如果缓存中获取的构造方法或者工厂方法不为空，并且mbd标记了构造函数参数已解析的话，则从缓存中获取已解析的构造函数参数
					argsToUse = mbd.resolvedConstructorArguments;
					if (argsToUse == null) {
						// 如果resolvedConstructorArguments为空的话，则从另外一个缓存preparedConstructorArguments进行获取
		...
		// 如果缓存resolvedConstructorOrFactoryMethod中不存在解析好的构造方法、或者(没有显式指定参数 && 缓存中也没有解析好的构造方法参数)
