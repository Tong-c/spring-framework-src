...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		try {
			// 从encodedResource中获取已经封装好的Resource对象，再根据Resource对象获取对应的输入流
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
		...
		try {
			// 将XML配置文件转换为Document文档对象，由documentLoader完成
			Document doc = doLoadDocument(inputSource, resource);

			// 从document文档中解析、注册bean的定义信息
	...
	protected Document doLoadDocument(InputSource inputSource, Resource resource) throws Exception {
		// 1. 获取对XML文件的验证模式: DTD or XSD, 看文档是否符合规范，元素和标签是否使用正确等
		// 2. 加载XML,得到对应的Document文档对象，并返回
		return this.documentLoader.loadDocument(inputSource, getEntityResolver(), this.errorHandler,
				// getValidationModeForResource检测验证模式: 判断是否包含DOCTYPE，如果包含就是DTD，否则就是XSD
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		// 通过反射创建文档读取器对象：DefaultBeanDefinitionDocumentReader
		BeanDefinitionDocumentReader documentReader = createBeanDefinitionDocumentReader();
		// 记录统计前BeanDefinition的加载个数
		int countBefore = getRegistry().getBeanDefinitionCount();
		// 加载及注册Bean对象
		documentReader.registerBeanDefinitions(doc, createReaderContext(resource));
		// 相减之后得到本次加载的BeanDefinition个数
