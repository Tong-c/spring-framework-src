...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	 ...
	 * @param earlySingletonObject 提前暴露的对象
	 */
	protected void addEarlySingletonObjects(String beanName, Object earlySingletonObject) {
	...
	public Object getSingleton(String beanName, ObjectFactory<?> singletonFactory) {
		...
		synchronized (this.singletonObjects) {
			...
			if (singletonObject == null) {
				...
				// 	private final Set<String> singletonsCurrentlyInCreation = Collections.newSetFromMap(new ConcurrentHashMap<>(16));
				// 标记当前 bean 被创建，将当前beanName加入到正在创建的bean的集合中，如果有构造函数注入的循环依赖会报错
				/**
				 * 构造器注入就是在这里抛出"Requested bean is currently in creation: Is there an unresolvable circular reference?"异常的。
				 * 大体过程如下： 假设 A -> B、 B -> A造成循环依赖，使用构造器注入
				 * 1、每次bean实例化前，会先将beanName加入到Set<String> singletonsCurrentlyInCreation = Collections.newSetFromMap(new ConcurrentHashMap<>(16))中【表示当前正在创建中的bean的集合】，例如getBean("a")的时候，会将"a"加入singletonsCurrentlyInCreation集合中【表示"a"正在创建过程中】；
				 * 2、实例化a：执行new A(B b)构造方法的时候，需要一个b对象，然后执行getBean("b")，将"b"加入singletonsCurrentlyInCreation集合中【表示"b"正在创建过程中】；
				 * 3、实例化b：执行new B(A a)的构造方法的时候，需要一个a对象，又会执行getBean("a")  -> 调用DefaultSingletonBeanRegistry#getSingleton(java.lang.String, org.springframework.beans.factory.ObjectFactory<?>)方法中的beforeSingletonCreation()方法；
				 * 4、beforeSingletonCreation()方法判断到当前需要的bean对象【"a"】已经在singletonsCurrentlyInCreation【当前正在创建中的bean的集合】中了，Spring就会抛出BeanCurrentlyInCreationException异常；
				 */
				...
				try {
					// 执行singletonFactory的回调，也就是真正帮我们创建对象的地方
					singletonObject = singletonFactory.getObject();
					newSingleton = true;
				}
					...
					// 创建单例后的操作：通过this.singletonsCurrentlyInCreation.remove(beanName)从正在创建的bean的集合中移除当前bean的记录
					// 	private final Set<String> singletonsCurrentlyInCreation = Collections.newSetFromMap(new ConcurrentHashMap<>(16));
