...
class ConfigurationClassEnhancer {
	...
	public Class<?> enhance(Class<?> configClass, @Nullable ClassLoader classLoader) {
		...
		// newEnhancer(): 创建Cglib Enhancer实例，使用Cglib生成一个增强之后的类
		// 通过enhancer.createClass()用增强器来生成超类的子类，如com.wsh.configuration.MyBeanConfig$$EnhancerBySpringCGLIB$$822c128f
	...
	private Enhancer newEnhancer(Class<?> configSuperClass, @Nullable ClassLoader classLoader) {
		...
		// 通过cglib代理的类在调用方法时，会通过CallbackFilter调用
//		private static final ConditionalCallbackFilter CALLBACK_FILTER = new ConditionalCallbackFilter(CALLBACKS);
//		private static final Callback[] CALLBACKS = new Callback[] {
				// BeanMethodInterceptor: 拦截任何{@Bean}注解的方法的调用,以确保正确处理Bean的语义范围和AOP代理等
//				new BeanMethodInterceptor(),
//				new BeanFactoryAwareMethodInterceptor(),
//				NoOp.INSTANCE
//		};
	...
	private static class BeanMethodInterceptor implements MethodInterceptor, ConditionalCallback {
		...
		public Object intercept(Object enhancedConfigInstance, Method beanMethod, Object[] beanMethodArgs,
			...
			// 首先通过反射从增强的@Configuration注解类中获取到工厂对象：beanFactory
			ConfigurableBeanFactory beanFactory = getBeanFactory(enhancedConfigInstance);
			// 然后通过方法获取beanName，默认为方法名，可以通过@Bean注解指定
			...
			// 检查当前bean是否是一个FactoryBean，如果是, 则创建一个代理,截取调用子类对象的getObject()方法并返回任何缓存的bean实例
			...
			// 判断当前执行的方法是否为正在执行的@Bean方法
			//因为存在在 student() 方法中调用 teacher() 方法
			//如果 teacher() 也有 @Bean 注解，那么这个返回值就是 false.
			...
			// 如果不满足上面 if，也就是在 student() 中调用的 teacher() 方法
